...
const LotteryInterface: React.FC = () => {
  ...
  const calculateTotalPrice = (raffles: number, checked:boolean): string => {
    return (BASE_PRICE * raffles + EXTRA * Number(checked)).toFixed(2);
  };
  

  const handleRaffleSelection = (count: number): void => {
      const calculatedPrice = BASE_PRICE * count;
      setTotalPrice
  };
    ...
    <Card className="left-column">
      <div className="left-column-content">
        ...
        <div className="extra-option">
          ...
          <label>
        <input type="checkbox" checked={extraSelected} onChange={(e) => onChange()} />
      </label>
